...
veh_cost_insurance = pd.concat([calculate_insurance(veh_cost, input_rate[i]) for i in range(7)])
...
df_powertrain = df.groupby(['region', 'year','powertrain_type']).agg(TCO_POWERTRAIN = ("TCO_percent", sum))

df_powertrain.reset_index().to_csv("data/output/TCO_Powertrain.csv", index=False)
