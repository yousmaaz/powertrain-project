...
def calculate_veh_cot_factor(row, factor=1.5): 
    try:
        if (math.isnan(row['veh_price_ihs_avg']) == False): 
            A = row['veh_price_ihs_avg'] - row['veh_price_km_min']
            B = row['veh_price_km'] - row['veh_price_km_min']
            F = row['veh_price_ihs_avg'] / factor
...
def get_ms_data(agg_volume_powertrain_per,veh_cost, tco_data):
  ...
  tco_data.drop_duplicates(['powertrain_type', 'region', 'year'], inplace = True)
  tco_data['ratio_volume_previous'] = tco_data.groupby(['region','powertrain_type'])['ratio_volume'].shift(1)
  tco_data['TCO_Percent_ihs']= tco_data['TCO_AVG_POWERTRAIN'] * tco_data['ratio_volume_previous']
  tco_data['TCO_AVG'] = tco_data.groupby(['region', 'year'])['TCO_Percent_ihs'].transform(sum)
#  s = tco_data[['region', 'year', 'TCO_AVG']].drop_duplicates()
# 
#  tco_data = pd.merge(tco_data, s, on = ['region', 'year'])
...
#  tco_data['TCO_AVG'] = tco_data.groupby(['region', 'year'])["TCO_AVG_POWERTRAIN"].transform(np.mean)
  ...
  tco_data = tco_data[['powertrain_type', 'region', 'year', 'TCO_AVG_POWERTRAIN', 'TCO_AVG','TCO_MIN', 'veh_price_km','veh_price_km_avg', 'ratio_volume', 'ratio_volume_previous', 'veh_price_km_min']].drop_duplicates()
  tco_data['TCO_VAR'] = tco_data.groupby(['powertrain_type', 'region'])['TCO_AVG_POWERTRAIN'].pct_change() 

  
  tco_data['veh_price_ihs']= tco_data['veh_price_km'] * tco_data['ratio_volume_previous']
